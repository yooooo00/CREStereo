...
if __name__ == "__main__":
    ...
    for filename in os.listdir(args.left):
        left = cv2.imread(os.path.join(args.left, filename))
        filename_right=filename.replace('left', 'right')
        right = cv2.imread(os.path.join(args.right, filename_right))

        # if left.shape != right.shape:
        #     print(f"{left.shape}{right.shape}The input images have inconsistent shapes.")
        ...
        cv2.imwrite(os.path.join(args.output,filename), disp_vis)
        print("Done! Result path:", os.path.abspath(os.path.join(args.output,filename)))

# python D:\projects\CREStereo\forward_dir.py --model_path "D:/projects/CREStereo/crestereo_eth3d.mge" --left D:/dataset/data/FallingThings/kitchen_0_result/center/left --right D:/dataset/data/FallingThings/kitchen_0_result/center/right --size 200x200 --output "D:/dataset/data/FallingThings/kitchen_0_result/output_CREStereo"